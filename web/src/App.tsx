...
import type { ActorSpec, Branch, FactState, OutputType, RunPhase, SceneArtifact, SceneDraft, SceneEvent, SceneOutput, SceneRecord, SceneSpec, WorldSnapshot } from "./types";
...
export default function App() {
  ...
  async function expandIdea() { await act("正在拆解想法", async () => { const result = await api.expandIdea(premise.trim()); setExpanded(normalizeSpec(result.scene_spec as SceneSpec & Record<string, unknown>, premise)); setQuestions(result.questions ?? []); setNotice("建议设定仍可编辑；确认后才会建立正式沙盘。"); }); }
  ...
  if (!location.id) return <Landing premise={premise} setPremise={setPremise} expanded={expanded} questions={questions} busy={busy} error={error} notice={notice} professional={professional} runtime={runtime} toggleMode={toggleMode} onExpand={expandIdea} onCreate={createScene} onCancel={() => setExpanded(null)} />;
...
function OutputView({ scene, snapshot, outputs, setOutputs, busy }: { scene: SceneSpec; snapshot: WorldSnapshot; outputs: SceneOutput[]; setOutputs: (outputs: SceneOutput[]) => void; busy: boolean }) {
  const [type, setType] = useState<OutputType>("screenplay"); const [chosen, setChosen] = useState<string[]>([]); const [edits, setEdits] = useState<Record<string, string>>({}); const events = snapshot.events;
  async function generate() { try { const output = await api.createOutput(scene.scene_id, snapshot.branch_id, type, chosen, snapshot.revision); setOutputs([...outputs, output]); } catch (reason) { alert(message(reason)); } }
