...
def main() -> int:
    ...
    textbox(s, 0.7, 1.6, 6.0, 5.2, [
        ["1  剧情结构层", {"size": 15, "bold": True, "color": ACCENT}],
        ["     本场契约 + 5 个剧情节拍 + 8 项剧情检查", {"size": 11.5}],
        ["     把轮流说话变成有冲突有收尾的一场戏", {"size": 11.5}],
        ["2  看得见 Agent 怎么演", {"size": 15, "bold": True, "color": ACCENT}],
        ["     点事件看 提议→检查→最终→状态变化；", {"size": 11.5}],
        ["     说话人/指向线/被对话人三者同时高亮", {"size": 11.5}],
        ["3  作者掌控", {"size": 15, "bold": True, "color": ACCENT}],
        ["     预览→锁定→提交（幂等+版本校验）；", {"size": 11.5}],
        ["     节拍由作者推进，没有自动提交路径", {"size": 11.5}],
        ["4  真模型 + 真素材（来源如实标注）", {"size": 15, "bold": True, "color": ACCENT}],
        ["     候选 source=llm；图片带提示词与时间戳；", {"size": 11.5}],
        ["     检查=规则、输出=确定性/润色 均写明", {"size": 11.5}],
        ["5  一句话 → 完整开场 + 舞台布局", {"size": 15, "bold": True, "color": ACCENT}],
        ["     华山→室外山景，矿洞→岩壁矿车，游轮→驾驶台", {"size": 11.5}],
    ])
    textbox(s, 7.0, 1.6, 5.8, 5.2, [
        ["1  Dramatic structure layer", {"size": 12.5, "bold": True, "color": ACCENT}],
        ["     scene contract + 5 beats + 8 dramatic checks", {"size": 10.5}],
        ["2  See how agents act", {"size": 12.5, "bold": True, "color": ACCENT}],
        ["     click an event: proposal → check → final", {"size": 10.5}],
        ["     speaker, talk line and addressee all highlight", {"size": 10.5}],
        ["3  The author stays in control", {"size": 12.5, "bold": True, "color": ACCENT}],
        ["     preview → lock → commit; beats advance only", {"size": 10.5}],
        ["     when the author says so; no auto-commit", {"size": 10.5}],
        ["4  Real model, real assets, honest labels", {"size": 12.5, "bold": True, "color": ACCENT}],
        ["     candidates source=llm; images with prompts/time;", {"size": 10.5}],
        ["     checks = rules; outputs labelled deterministic", {"size": 10.5}],
        ["5  One line → opening + stage layout", {"size": 12.5, "bold": True, "color": ACCENT}],
        ["     mountain / mine / ship bridge adapt per genre", {"size": 10.5}],
    ])
