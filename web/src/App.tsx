...
export default function App() {
  ...
  const beatTotal = assessment?.beat_total ?? beats.length;
  ...
  async function advanceBeat() {
    ...
    await act("正在进入下一拍", async () => {
      const next = await api.advanceBeat(scene.scene_id, branchId, formal.revision);
      setDraft(null);
      await load(scene.scene_id, branchId);
      setNotice(`已进入第 ${(next.beat_state?.index ?? 0) + 1} / ${next.beat_total} 拍：${next.beat?.purpose ?? ""}。请重新生成候选。`);
    });
  }
